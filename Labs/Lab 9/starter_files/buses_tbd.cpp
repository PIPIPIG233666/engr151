...
Bus bus(string n, vector<string> s, int i) {
...
bool Bus::getOn(Rider rider) {
  //TODO: Fix the logic!
  if (/*is there space on the bus*/ bus.capacity > 50) {
    return false;
  } else if (/*is the bus currently at the rider start*/bus.stops.at(bus.currentStop) != riderDestinations.at(currentStop) ) {
    return false;
  } else if (/*is the riders destination on this route*/ !Bus::stopsAt(riderDestinations)) {
    return false;
  } else {
    //let the rider on
